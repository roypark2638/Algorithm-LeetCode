...
that contains the speeds of riders wearing blue shirts. Each rider is represented by a signle positive integer,
...
Write a function that returns the maximum possible total speed or the minimum possible total speed of all of
 the tandem bicycles being ridden based on an input parameter, fastest. If fastest = true,
...
"Total speed" is defined as the sum of the speeds of all the tandem bicycles being ridden. For example,
 if there are 4 riders (2 red-shirt riders and 2 blue-shirt riders) who have speeds of 1,3,4,5, and if they're paired
...
sort one array ascending and another array descending
iterate the array
if the fastest = true:
    then get total of max(red, blue)
else:
    then get total of min(red, blue)
